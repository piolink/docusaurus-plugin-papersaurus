...
async function generatePdfFiles(outDir, pluginOptions, { siteConfig, plugins }, language) {
    var _a;
    const queue = new p_queue_1.default({ concurrency: (_a = pluginOptions.concurrency) !== null && _a !== void 0 ? _a : 1 });
    ...
    for (const versionInfo of docPlugin.content.loadedVersions) {
        ...
        for (const [i, sidebarName] of pluginOptions.sidebarNames.entries()) {
            ...
            if (sidebar) {
                ...
                await createPdfFilesRecursive(queue, language, rootCategory, [], [], versionInfo, pluginOptions, siteConfig, versionBuildDir, versionPdfPath, browser, siteAddress, productTitle, productVersion);
...
async function createPdfFilesRecursive(queue, language, sideBarItem, parentTitles, parentIds, version, pluginOptions, siteConfig, buildDir, pdfPath, browser, siteAddress, productTitle, productVersion) {
    let articles = [];
    switch (sideBarItem.type) {
        case 'category': {
            ...
            for (const categorySubItem of sideBarItem.items) {
                const subDocs = await createPdfFilesRecursive(queue, language, categorySubItem, newParentTitles, newParentIds, version, pluginOptions, siteConfig, buildDir, pdfPath, browser, siteAddress, productTitle, productVersion);
    ...
    if (articles.length > 0) {
        queue.add(async () => {
            await createPdfFromArticles(language, documentTitle, productVersion || version.label, pdfFilename, articles, pluginOptions, siteConfig, buildDir, browser, siteAddress);
        });
...
async function createPdfFromArticles(language, documentTitle, documentVersion, pdfName, articleList, pluginOptions, siteConfig, buildDir, browser, siteAddress) {
    ...
    await coverPage.setContent(pluginOptions.getPdfCoverPage(siteConfig, pluginOptions, documentTitle, documentVersion, language, siteAddress), {
  ...
  <html lang="${language}">
    ...
    const parsedData = await (0, pdf_parse_new_1.default)(await generateContentPdf(), { verbosityLevel: 0 });
    ...
    async function generateContentPdf() {
        ...
        return await page.pdf({
            format: 'a4',
            headerTemplate: pluginOptions.getPdfPageHeader(siteConfig, pluginOptions, documentTitle, documentVersion, language, siteAddress),
            footerTemplate: pluginOptions.getPdfPageFooter(siteConfig, pluginOptions, documentTitle, documentVersion, language, siteAddress),
            displayHeaderFooter: true,
            printBackground: true,
            scale: 1,
    ...
    async function mergeMultiplePDF(pdfFiles, name) {
